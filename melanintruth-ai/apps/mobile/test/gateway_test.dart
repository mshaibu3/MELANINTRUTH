...
void main() {
  test(
    'runs signed upload and governed analysis with bounded PUT retry',
    () async {
      ...
      final gateway = HttpMelaninTruthGateway(
        ...
        uploadRetryPolicy: RetryPolicy(maxAttempts: 3, delay: (_) async {}),
        telemetry: telemetry,
      );
      ...
      expect(telemetry.records.map((record) => record.event), [
      ...
      expect(
        ...
        everyElement(
          isIn(const ['attempt', 'outcome', 'status_class', 'stage']),
        ),
      );
    },
  );
